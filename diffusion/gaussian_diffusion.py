...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l2_loss = th.nn.HuberLoss()  # must be None for handling mask later on.
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        ...
        return (
            ...
            - _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * eps
        )
    ...
    def p_sample(
        ...
        if const_noise:
            noise = th.randn(1, 1, x.shape[-1], device=x.device)
    ...
    def p_sample_loop(
        ...
        for i, sample in enumerate(self.p_sample_loop_progressive(
            ...
            # prediction = sample['pred_xstart']
            # prediction = prediction.squeeze()
            # prediction = prediction.detach().cpu().numpy()
            # prediction = RIG_SCALER.inverse_transform(prediction)
            # np.save(f'/scratch/2176327/diff_steps/test_{i + 1}', prediction)
